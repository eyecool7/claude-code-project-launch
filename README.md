...
계획서 기반 Claude Code 프로젝트 자동 세팅 — 한 장의 계획서에서 CLAUDE.md + 19개 설정 파일까지.
...
- Claude Code로 프로젝트를 시작하고 싶은데, 뭘 먼저 세팅해야 할지 모르는 분
- CLAUDE.md를 58줄로 쓸지 500줄로 쓸지 고민하는 분
- 서브에이전트, 팀모드, 스킬, MCP — 뭘 골라야 할지 모르는 분
- 세팅까지는 했는데, 실제 구축은 뭐부터 해야 할지 막막한 분
- 며칠 뒤 다시 돌아왔을 때 "어디까지 했더라?" 하고 헤매는 분
- 세션이 바뀔 때마다 맥락을 잃어서 같은 실수를 반복하는 분
- 범용 템플릿 복붙은 그만하고, 내 프로젝트에 맞는 세팅을 원하는 분

---

## 어떻게 작동하나요?

4단계 명령어를 따라가면 프로젝트에 맞는 설정이 자동 생성됩니다:

1. **계획서 작성** — claude.ai에서 인터뷰를 통해 계획서를 만들어요
2. **계획서 고도화** — Claude Code에서 기술 심층 리뷰 + 스킬/MCP 검색까지
3. **프로젝트 세팅** — 계획서 기반으로 CLAUDE.md + 19개 설정 파일 자동 생성
4. **구축 시작** — 매 세션마다 Phase 판별 + 세션 목표 출력
...
`/plan` → `/refine` → `/setup` → `/build` 순서대로 실행하면 프로젝트에 맞는 CLAUDE.md, rules, skills, agents, hooks가 자동 생성됩니다. 매 세션마다 Phase 기반 구축 킥오프가 제공돼요.
...
새 세션에서 `/build` 실행하면 이전 기록을 전부 읽고 이어서 Phase를 판별합니다.
...
`/project-launch:build` → Phase 판별 + 세션 목표 출력

매 세션 시작 시 실행하면 돼요. 며칠 후 재진입해도 `decisions.md`, `lessons.md`, `git log`를 읽고 이어서 진행할 Phase를 판별합니다.
